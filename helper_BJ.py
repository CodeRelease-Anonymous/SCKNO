    ...
    def get_loss(self, x, future_pred, history_pred, reconstructions, node_fit_err, invariant_err, loss_metric, IM):
        ...
        if self.rec:
            ...
            rec_mae, rec_rmse, rec_mape = metric_all(
                [rec_x[..., 0:2], rec_x[..., 4:6], rec_x[..., 2:4], rec_x[..., 6:7]],
                [x[:, :, :self.num_for_predict, 0:2],
                 x[:, :, :self.num_for_predict, 4:6],
                 x[:, :, :self.num_for_predict, 2:4],
                 x[:, :, :self.num_for_predict, 6:7]])
        ...
        if self.hispred:
            ...
            hispred_mae, hispred_rmse, hispred_mape = metric_all(
                [hispred_x[..., 0:2], hispred_x[..., 4:6], hispred_x[..., 2:4], hispred_x[..., 6:7]],
                [x[:, :, 1:self.num_for_predict, 0:2],
                 x[:, :, 1:self.num_for_predict, 4:6],
                 x[:, :, 1:self.num_for_predict, 2:4],
                 x[:, :, 1:self.num_for_predict, 6:7]])
        ...
        if self.pred:
            ...
            pred_mae, pred_rmse, pred_mape = metric_all(
                    [pred_x[:, :, -self.num_for_target:, 0:2],
                     pred_x[:, :, -self.num_for_target:, 4:6],
                     pred_x[:, :, -self.num_for_target:, 2:4],
                     pred_x[:, :, -self.num_for_target:, 6:7]],
                    [x[:, :, -self.num_for_target:, 0:2],
                     x[:, :, -self.num_for_target:, 4:6],
                     x[:, :, -self.num_for_target:, 2:4],
                     x[:, :, -self.num_for_target:, 6:7]])
