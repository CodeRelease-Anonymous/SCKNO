...
def model_val(runid, engine, dataloader, device, logger, cfg, epoch, loss_list, data_dim_list):
    ...
    for iter, (x, pos) in val_tqdm_loader:
        ...
        record(valid_rec_mae, valid_rec_rmse, valid_rec_mape, rec_set[0], rec_set[1], rec_set[2])
        record(valid_pred_mae, valid_pred_rmse, valid_pred_mape, pred_set[0], pred_set[1], pred_set[2])
    ...
    log = 'Epoch: {:03d}, Vaild Total Loss: {:.4f} Learning rate: {}\n' \
          ...
          'Vaild Rec  Bike  MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f} \n' \
          'Vaild Rec  Taxi  MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f} \n' \
          'Vaild Rec  Bus   MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f} \n' \
          'Vaild Rec  Speed MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f} \n\n' \
          'Vaild Hispred Bike  MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
          'Vaild Hispred Taxi  MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
          'Vaild Hispred Bus   MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
          'Vaild Hispred Speed MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
          'Vaild Pred Bike  MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
          'Vaild Pred Taxi  MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
          'Vaild Pred Bus   MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
    ...
    logger.info(log.format(epoch, mvalid_loss[0], str(engine.scheduler.get_lr()),
                           ...
                           np.mean(valid_rec_mae['bike']), np.mean(valid_rec_rmse['bike']),
                           np.mean(valid_rec_mape['bike']),
                           np.mean(valid_rec_mae['taxi']), np.mean(valid_rec_rmse['taxi']),
                           np.mean(valid_rec_mape['taxi']),
                           np.mean(valid_rec_mae['bus']), np.mean(valid_rec_rmse['bus']),
                           np.mean(valid_rec_mape['bus']),
                           np.mean(valid_rec_mae['speed']), np.mean(valid_rec_rmse['speed']),
                           np.mean(valid_rec_mape['speed']),

                           np.mean(valid_hispred_mae['bike']), np.mean(valid_hispred_rmse['bike']),
                           np.mean(valid_hispred_mape['bike']),
                           np.mean(valid_hispred_mae['taxi']), np.mean(valid_hispred_rmse['taxi']),
                           np.mean(valid_hispred_mape['taxi']),
                           np.mean(valid_hispred_mae['bus']), np.mean(valid_hispred_rmse['bus']),
                           np.mean(valid_hispred_mape['bus']),
                           np.mean(valid_hispred_mae['speed']), np.mean(valid_hispred_rmse['speed']),
                           np.mean(valid_hispred_mape['speed']),

                           np.mean(valid_pred_mae['bike']), np.mean(valid_pred_rmse['bike']),
                           np.mean(valid_pred_mape['bike']),
                           np.mean(valid_pred_mae['taxi']), np.mean(valid_pred_rmse['taxi']),
                           np.mean(valid_pred_mape['taxi']),
                           np.mean(valid_pred_mae['bus']), np.mean(valid_pred_rmse['bus']),
                           np.mean(valid_pred_mape['bus']),
...
def model_test(runid, engine, dataloader, device, logger, cfg, loss_list, data_dim_list, mode='Test', ):
    ...
    for iter, (x, pos) in test_tqdm_loader:
        ...
        record(test_rec_mae, test_rec_rmse, test_rec_mape, rec_set[0], rec_set[1], rec_set[2])
        record(test_pred_mae, test_pred_rmse, test_pred_mape, pred_set[0], pred_set[1], pred_set[2])
    ...
    log = 'Test Total Loss: {:.4f}\n' \
          ...
          'Test Hispred Bike  MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
          'Test Hispred Taxi  MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
          'Test Hispred Bus   MAE: {:.4f}\t\t\tRMSE: {:.4f}\t\t\tMAPE: {:.4f}\n' \
    ...
    logger.info(log.format(mtest_loss[0],
                           ...
                           np.mean(test_rec_mae['bike']), np.mean(test_rec_rmse['bike']),
                           np.mean(test_rec_mape['bike']),
                           np.mean(test_rec_mae['taxi']), np.mean(test_rec_rmse['taxi']),
                           np.mean(test_rec_mape['taxi']),
                           np.mean(test_rec_mae['bus']), np.mean(test_rec_rmse['bus']),
                           np.mean(test_rec_mape['bus']),
                           np.mean(test_rec_mae['speed']), np.mean(test_rec_rmse['speed']),
                           np.mean(test_rec_mape['speed']),

                           np.mean(test_hispred_mae['bike']), np.mean(test_hispred_rmse['bike']),
                           np.mean(test_hispred_mape['bike']),
                           np.mean(test_hispred_mae['taxi']), np.mean(test_hispred_rmse['taxi']),
                           np.mean(test_hispred_mape['taxi']),
                           np.mean(test_hispred_mae['bus']), np.mean(test_hispred_rmse['bus']),
                           np.mean(test_hispred_mape['bus']),
                           np.mean(test_hispred_mae['speed']), np.mean(test_hispred_rmse['speed']),
                           np.mean(test_hispred_mape['speed']),

                           np.mean(test_pred_mae['bike']), np.mean(test_pred_rmse['bike']),
                           np.mean(test_pred_mape['bike']),
                           np.mean(test_pred_mae['taxi']), np.mean(test_pred_rmse['taxi']),
                           np.mean(test_pred_mape['taxi']),
                           np.mean(test_pred_mae['bus']), np.mean(test_pred_rmse['bus']),
                           np.mean(test_pred_mape['bus']),
