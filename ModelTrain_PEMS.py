...
from ModelTest_PEMS import model_val, model_test
...
def baseline_train(runid,
    ...
    for epoch in range(begin_epoch, begin_epoch + epochs + 1):
        ...
        for iter, (x, pos) in train_tqdm_loader:
            ...
            record(train_rec_mae, train_rec_rmse, train_rec_mape, rec_set[0], rec_set[1], rec_set[2], mode="PeMS")
            record(train_pred_mae, train_pred_rmse, train_pred_mape, pred_set[0], pred_set[1], pred_set[2], mode="PeMS")
            record(train_hispred_mae, train_hispred_rmse, train_hispred_mape, hispred_set[0], hispred_set[1], hispred_set[2], mode="PeMS")
        ...
        if (epoch - 1) % cfg['train']['print_every'] == 0:
            ...
            # lose_list = ['total', 'pred', 'hispred', 'recon', 'invariant', 'nodefit', 'metric', 'L1', 'KL']
            ...
            logger.info(log.format(epoch, mtrain_loss[0], str(engine.scheduler.get_lr()), str(kl_weight),
                                   ...
                                   np.mean(train_rec_mae['speed']),
                                   np.mean(train_rec_rmse['speed']),
                                   np.mean(train_rec_mape['speed']),

                                   np.mean(train_hispred_mae['speed']),
                                   np.mean(train_hispred_rmse['speed']),
                                   np.mean(train_hispred_mape['speed']),

                                   np.mean(train_pred_mae['speed']),
                                   np.mean(train_pred_rmse['speed']),
